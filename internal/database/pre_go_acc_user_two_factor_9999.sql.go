...
// versions:
//   sqlc v1.29.0
// source: pre_go_acc_user_two_factor_9999.sql
...
const getUserTwoFactorMethod = `-- name: GetUserTwoFactorMethod :many
SELECT two_factor_id, user_id, two_factor_auth_type, two_factor_auth_secret,
       two_factor_phone, two_factor_email,
       two_factor_is_active, two_factor_created_at, two_factor_updated_at
FROM pre_go_acc_user_two_factor_9999
...
func (q *Queries) GetUserTwoFactorMethod(ctx context.Context, userID uint32) ([]PreGoAccUserTwoFactor9999, error) {
	rows, err := q.db.QueryContext(ctx, getUserTwoFactorMethod, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
