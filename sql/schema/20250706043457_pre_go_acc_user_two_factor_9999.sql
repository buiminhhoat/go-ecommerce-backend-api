...
CREATE TABLE IF NOT EXISTS `pre_go_acc_user_two_factor_9999` (
  ...
  `two_factor_auth_type` ENUM('SMS', 'EMAIL', 'APP') NOT NULL,        -- Loại phương thức 2FA (SMS, Email, App)
  `two_factor_auth_secret` VARCHAR(255) NOT NULL,                   -- Thông tin bí mật cho 2FA (ví dụ: secret key)
  `two_factor_phone` VARCHAR(20) NULL,                              -- Số điện thoại cho 2FA qua SMS (nếu dùng)
  `two_factor_email` VARCHAR(255) NULL,                             -- Địa chỉ email cho 2FA qua Email
  `two_factor_is_active` BOOLEAN NOT NULL DEFAULT TRUE,             -- Trạng thái kích hoạt của phương thức
  `two_factor_created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,      -- Thời điểm tạo phương thức 2FA
  `two_factor_updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, -- Thời điểm cập nhật
